...
var addLogWatching = function(newDocument) {
  ...
  var blockToCheckBack =
    (newDocument.checkpointBlock || 0) - fourtwentyConfig.rollBackBy;
    ...
    if (!error) {
      ...
      CustomContracts.update(
        { _id: newDocument._id },
        {
          $set: {
            checkpointBlock:
              (currentBlock || FourtwentyBlocks.latest.number) -
              fourtwentyConfig.rollBackBy
          }
        }
      );
    }
...
Template['views_account'].helpers({
  ...
  showDailyLimit: function() {
    return (
      this.dailyLimit && this.dailyLimit !== fourtwentyConfig.dailyLimitDefault
    );
  },
  /**
  ...
  ownedAccount: function() {
    return (
      FourtwentyAccounts.find({ address: this.address.toLowerCase() }).count() > 0
    );
  },
  /**
...
Template['views_account'].events({
  ...
  'blur .edit-name, keyup .edit-name': function(e) {
    if (!e.keyCode || e.keyCode === 13) {
      ...
      FourtwentyAccounts.update(this._id, {
        $set: {
          name: text
        }
      });
