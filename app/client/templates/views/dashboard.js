...
Template['views_dashboard'].helpers({
  ...
  /**
    ...
    */
  accounts: function() {
    // balance need to be present, to show only full inserted accounts (not ones added by haze.requestAccount)
  ...
  hasAccounts: function() {
    return FourtwentyAccounts.find().count() > 0;
  },
  /**
    Are there any accounts?
  ...
  hasMinimumBalance: function() {
    ...
    _.each(_.pluck(FourtwentyAccounts.find({}).fetch(), 'balance'), function(bal) {
...
Template['views_dashboard'].events({
  /**
    Request to create an account in haze
  ...
  'click .create.account': function(e) {
    ...
    haze.requestAccount(function(e, accounts) {
      if (!e) {
