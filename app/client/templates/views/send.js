...
/**
The default smoke to provide for estimates. This is set manually,
...
@property defaultEstimateSmoke
*/
var defaultEstimateSmoke = 50000000;
...
*/
var checkOverDailyLimit = function(address, marley, template) {
  ...
  if (
    ...
    account.dailyLimit !== fourtwentyConfig.dailyLimitDefault &&
    Number(marley) !== 0
  ) {
    // check whats left
    ...
    if (restDailyLimit.lt(new BigNumber(marley, 10)))
      TemplateVar.set(
        'dailyLimitText',
        new Spacebars.SafeString(
          TAPi18n.__('wallet.send.texts.overDailyLimit', {
            limit: FourtwentyTools.formatBalance(restDailyLimit.toString(10)),
            total: FourtwentyTools.formatBalance(account.dailyLimit),
...
var getDataField = function() {
  ...
  if (selectedToken && selectedToken !== '420coin') {
...
var estimationCallback = function(e, res) {
  ...
  console.log('Estimated smoke: ', res, e);

  if (!e && res) {
    TemplateVar.set(template, 'estimatedSmoke', res);

    // show note if its defaultEstimateSmoke, as the data is not executeable
    if (res === defaultEstimateSmoke)
...
Template['views_send'].onCreated(function() {
  ...
  template.autorun(function(c) {
    var unit = FourtwentyTools.getUnit();

    if (!c.firstRun && TemplateVar.get('selectedToken') === '420coin') {
      TemplateVar.set(
        'amount',
        FourtwentyTools.toMarley(
...
Template['views_send'].onRendered(function() {
  ...
  template.autorun(function(c) {
    ...
    // 420coin tx estimation
    if (tokenAddress === '420coin') {
      if (FourtwentyAccounts.findOne({ address: address }, { reactive: false })) {
        web3.fourtwenty.estimateSmoke(
          {
            from: address,
            to: to,
            value: amount,
            data: data,
            smoke: defaultEstimateSmoke
          },
      ...
      TokenContract.methods.transfer(to, amount).estimateSmoke(
        {
          from: address,
          smoke: defaultEstimateSmoke
        },
...
Template['views_send'].helpers({
  ...
  selectedAccount: function() {
    return Helpers.getAccountByAddress(
  ...
  tokenSelectedAttr: function(token) {
    return TemplateVar.get('selectedToken') === token ? { checked: true } : {};
  },
  /**
    Get all tokens
  ...
  hasTokens: function() {
    ...
    return (
      TemplateVar.get('selectedAction') === 'send-funds' &&
      !!Tokens.findOne(query, { field: { _id: 1 } })
    );
  },
  /**
  ...
  total: function(fourtwentycoin) {
    ...
    // 420coin
    var smokeInMarley =
      TemplateVar.getFrom('.dapp-select-smoke-price', 'smokeInMarley') || '0';

    if (TemplateVar.get('selectedToken') === '420coin') {
      amount =
        selectedAccount && selectedAccount.owners
          ? amount
          : new BigNumber(amount, 10).plus(new BigNumber(smokeInMarley, 10));
    } else {
      amount = new BigNumber(smokeInMarley, 10);
    }
    return amount;
  },
  /**
  ...
  tokenTotal: function() {
    ...
    if (!_.isFinite(amount) || !token) return '0';

    return Helpers.formatNumberByDecimals(amount, token.decimals);
  },
  /**
    Returns the total amount - the fee paid to send all 420coin/coins out of the account
      ...
      amount = selectedAccount.owners
        ...
        : web3.utils.BN.max(
            ...
            new web3.utils.BN(selectedAccount.balance).sub(
              new web3.utils.BN(smokeInMarley)
            )
          ).toString(10);
    } else {
  ...
  timeText: function() {
    return TAPi18n.__(
      'wallet.send.texts.timeTexts.' +
        (
          (Number(
            TemplateVar.getFrom('.dapp-select-smoke-price', 'feeMultiplicator')
          ) +
            5) /
          2
        ).toFixed(0)
...
Template['views_send'].events({
  ...
  /**
    ...
    @event click .token-fourtwentycoin
    */
  'click .token-fourtwentycoin': function(e, template) {
    TemplateVar.set('selectedToken', '420coin');
  ...
  'keyup input[name="amount"], change input[name="amount"], input input[name="amount"]': function(
    ...
    // 420coin
    if (TemplateVar.get('selectedToken') === '420coin') {
      var marley = FourtwentyTools.toMarley(e.currentTarget.value.replace(',', '.'));

      TemplateVar.set('amount', marley || '0');
      ...
      amount = new BigNumber(amount, 10)
        .times(Math.pow(10, token.decimals || 0))
        .floor()
        .toString(10);
  ...
  'submit form': function(e, template) {
    var amount = TemplateVar.get('amount') || '0',
      tokenAddress = TemplateVar.get('selectedToken'),
      to = TemplateVar.getFrom('.dapp-address-input .to', 'value'),
      smokePrice = TemplateVar.getFrom('.dapp-select-smoke-price', 'smokePrice'),
      estimatedSmoke = TemplateVar.get('estimatedSmoke'),
    ...
    if (selectedAccount && !TemplateVar.get('sending')) {
      // set smoke down to 21 000, if its invalid data, to prevent high smoke usage.
      if (estimatedSmoke === defaultEstimateSmoke || estimatedSmoke === 0)
        estimatedSmoke = 22000;

      // if its a wallet contract and tokens, don't need to remove the smoke addition on send-all, as the owner pays
      if (sendAll && (selectedAccount.owners || tokenAddress !== '420coin'))
        sendAll = false;
      ...
      console.log('Providing smoke: ', estimatedSmoke, sendAll ? '' : ' + 100000');
      ...
      if (
        ...
        (!selectedAccount.owners || tokenAddress === '420coin')
      )
        return GlobalNotification.warning({
      ...
      var sendTransaction = function(estimatedSmoke) {
        // show loading
        TemplateVar.set(template, 'sending', true);

        // use smoke set in the input field
        estimatedSmoke =
          estimatedSmoke || Number($('.send-transaction-info input.smoke').val());
        console.log('Finally choosen smoke', estimatedSmoke);
        ...
        if (contracts['ct_' + selectedAccount._id]) {
          ...
          console.log('Smoke Price: ' + smokePrice);
      ...
      // SHOW CONFIRMATION WINDOW when NOT HAZE BROWSER
      if (typeof haze === 'undefined') {
        console.log('estimatedSmoke: ' + estimatedSmoke);

        FourtwentyElements.Modal.question(
          {
            ...
            data: {
              ...
              amount: amount,
              smokePrice: smokePrice,
              estimatedSmoke: estimatedSmoke,
              estimatedSmokePlusAddition: sendAll
                ? estimatedSmoke
                : estimatedSmoke + 100000, // increase the provided smoke by 100k
              data: data
            },
            ok: sendTransaction,
            cancel: true
        ...
        // LET HAZE HANDLE the CONFIRMATION
      } else {
        sendTransaction(sendAll ? estimatedSmoke : estimatedSmoke + 100000);
      }
    }
  }
});
