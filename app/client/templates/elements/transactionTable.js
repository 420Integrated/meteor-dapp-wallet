...
@type Number
*/
var blocksForConfirmation = fourtwentyConfig.requiredConfirmations;
  ...
  items: function() {
    ...
    if (searchQuery) {
      ...
      items = _.filter(items, function(item) {
        ...
        if (
          pattern.test(
            FourtwentyTools.formatBalance(item.value, '0,0.00[000000] unit')
          )
        )
          return item;
...
Template['elements_transactions_row'].helpers({
  ...
  incomingTx: function(account) {
    var account =
      FourtwentyAccounts.findOne({ _id: account }) ||
      Wallets.findOne({ _id: account });
    return !!(
      (account && this.from !== account.address) ||
      (!account &&
        (FourtwentyAccounts.findOne({ address: this.to }) ||
  ...
  transactionType: function() {
    ...
    else if (this.outOfSmoke)
      return TAPi18n.__('wallet.transactions.types.outOfSmoke');
  ...
  unConfirmed: function() {
    if (!this.blockNumber || !FourtwentyBlocks.latest.number)
      return {
        confirmations: 0,
        percent: 0
      };

    var currentBlockNumber = FourtwentyBlocks.latest.number + 1,
      ...
      var sendConfirmation = function(owner) {
        ...
        if ((wallet = Wallets.findOne({ address: owner }))) {
          ...
          contracts['ct_' + wallet._id].execute(
            ...
            0,
            confirmData,
            { from: wallet.owners[0], smoke: 200000 },
            callback
          );
        } else {
          confirmFunc.sendTransaction(
            _this.operation,
            { from: owner, smoke: 200000 },
            callback
          );
        }
      };
