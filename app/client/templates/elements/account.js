...
Template['elements_account'].helpers({
  ...
  account: function() {
    return (
      FourtwentyAccounts.findOne(this.account) ||
  ...
  new: function() {
    if (this.new) {
      ...
      Meteor.setTimeout(function() {
        FourtwentyAccounts.update(id, { $unset: { new: '' } });
  ...
  creating: function() {
    ...
    var belowReorgThreshold =
      ...
      FourtwentyBlocks.latest.number - (this.creationBlock - 1);
    var blockNumberCheck =
      FourtwentyBlocks.latest.number - (this.creationBlock - 1) >= 0;
  ...
  unConfirmed: function() {
    ...
    var currentBlockNumber = FourtwentyBlocks.latest.number,
