...
Template['elements_compileContract'].onCreated(function() {
  ...
  this.autorun(function() {
    ...
    if (selectedType && selectedType === 'source-code' && selectedContract) {
      ...
      try {
        txData = new web3.fourtwenty.Contract(selectedContract.jsonInterface)
          .deploy({
  ...
  this.aceEditor.getSession().on(
    ...
    _.debounce(function(e) {
      ...
      Meteor.setTimeout(function(argument) {
        web3.fourtwenty.compile.solidity(sourceCode, function(
          error,
          compiledContracts
        ) {
          ...
          if (!error) {
            compiledContracts = _.map(compiledContracts, function(
              contract,
              name
            ) {
              ...
              if (constructor) {
                constructor.inputs = _.map(
                  constructor.inputs,
                  Helpers.createTemplateDataFromInput
                );
              } else {
                constructor = {
                  inputs: []
