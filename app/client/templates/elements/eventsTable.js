  ...
  items: function() {
    ...
    if (searchQuery) {
      ...
      items = _.filter(items, function(item) {
        // search from address
        if (pattern.test(item.event)) return item;

        // search to address
        if (pattern.test(item.address)) return item;

        // search to return values
        if (
          _.find(item.args, function(value, name) {
            return pattern.test(value) || pattern.test(name);
          })
        )
          return item;
