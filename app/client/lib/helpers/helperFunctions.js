...
Helpers.getDefaultContractExample = function(withoutPragma) {
  ...
  if (withoutPragma) {
    ...
    // Keep this for now as the Haze-API object will only be availabe from Haze version >= 0.8.9
    // so that older versions that will query code from wallet.420integrated.com won't use broken example code.
    if (typeof haze !== 'undefined' && haze.solidity && haze.solidity.version) {
      solcVersion = haze.solidity.version;
    } else {
      solcVersion = '0.4.6';
    }
...
Helpers.getOwnedAccountFrom = function(accountList) {
  ...
  var accounts = FourtwentyAccounts.find({}, { sort: { balance: 1 } }).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals) {
  ...
  return FourtwentyTools.formatNumber(
...
Helpers.checkChain = function(callback) {
  ...
  web3.fourtwenty.getCode(originalContractAddress, function(e, code) {
...
Helpers.isWatchOnly = function(id) {
  return !Wallets.findOne({
    _id: id,
    owners: { $in: _.pluck(FourtwentyAccounts.find({}).fetch(), 'address') }
  });
};
...
Helpers.showNotification = function(i18nText, values, callback) {
  ...
  if (typeof haze !== 'undefined') haze.sounds.bip();
};
...
/**
Gets the docuement matching the given addess from the FourtwentyAccounts or Wallets collection.
...
Helpers.getAccountByAddress = function(address, reactive) {
  ...
  return (
    FourtwentyAccounts.findOne(query, options) ||
...
/**
Gets the docuement matching the given query from the FourtwentyAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
  ...
  return FourtwentyAccounts.find(query, options)
    .fetch()
    .concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the FourtwentyAccounts or Wallets collection and returns its name or address.
...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
  ...
  var unit = unit || FourtwentyTools.getUnit(),
    format = '0,0.00';
  ...
  if (
    ...
    var price = new BigNumber(String(web3.fromMarley(value, '420coin')), 10).times(
      exchangeRates[unit].price
    );
    return FourtwentyTools.formatNumber(price, format) + ' ' + unit.toUpperCase();
  } else {
    return FourtwentyTools.formatBalance(value, format + '[0000000000000000] UNIT');
  }
};
...
Helpers.getENSName = function(address, callback) {
  ...
  var ensContract = new web3.fourtwenty.Contract(ensContractAbi, ensAddress);
  var resolverContract = new web3.fourtwenty.Contract(resolverContractAbi);
