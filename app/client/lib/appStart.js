...
var showModal = function() {
  ...
  Meteor.setTimeout(function() {
    // if in haze, tell to start g420, otherwise start with RPC
    var g420RPC = window.haze
      ? 'g420'
      : 'g420 --ws --wsorigins "' +
...
var connect = function() {
  web3.fourtwenty.net
    ...
    .then(function(isListening) {
      if (isListening) {
        // only start app operation, when the node is not syncing (or the fourtwenty_syncing property doesn't exists)
        web3.fourtwenty
          .isSyncing()
          .then(function(syncing) {
            if (!syncing) {
