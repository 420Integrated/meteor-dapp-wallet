...
/**
Config for the 420coin connector
...
fourtwentyConfig = {
  /**
    ...
    @property fourtwentyConfig.rollBackBy
    */
  rollBackBy: 0,
  /**
    Number of blocks to confirm a wallet

    @property fourtwentyConfig.requiredConfirmations
    */
  requiredConfirmations: 12,
  /**
    The default daily limit used for simple accounts

    @property fourtwentyConfig.dailyLimitDefault
    */
...
var checkNetwork = function() {
  web3.fourtwenty.getBlock(0).then(function(block) {
    switch (block.hash) {
...
connectToNode = function() {
  ...
  FourtwentyAccounts.init();
  FourtwentyBlocks.init();

  FourtwentyTools.ticker.start({
    extraParams: typeof haze !== 'undefined' ? 'Haze-' + haze.version : '',
    currencies: ['BTC', 'USD', 'EUR', 'BRL', 'GBP']
  });

  if (FourtwentyAccounts.find().count() > 0) {
    checkForOriginalWallet();
  }

  // FourtwentyBlocks.detectFork(function(oldBlock, block){
  ...
  //         if(wallet.creationBlock + fourtwentyConfig.requiredConfirmations >= block.number)
...
resetWallet = function function_name(argument) {
  ...
  web3.fourtwenty.clearSubscriptions();
