    ...
    setTimeout(function() {
      _.each(wallet.owners, function(owner) {
        contract.hasConfirmed(conf.operation, owner, function(e, res) {
          if (!removed && !e) {
            if (res) {
              PendingConfirmations.update(confirmationId, {
                $addToSet: { confirmedOwners: owner }
              });
            } else {
              PendingConfirmations.update(confirmationId, {
                $pull: { confirmedOwners: owner }
              });
            }
            ...
            if (
              pendingConf &&
              (!pendingConf.confirmedOwners.length ||
                Number(wallet.requiredSignatures) ===
                  pendingConf.confirmedOwners.length)
            ) {
...
observePendingConfirmations = function() {
  /**
    ...
    @constructor
    */
  collectionObservers[collectionObservers.length] = PendingConfirmations.find(
    {}
  ).observe({
    /**
