...
var getPeerCount = function() {
  web3.fourtwenty.net.getPeerCount().then(function(peerCount) {
...
updateBalances = function() {
  ...
  var allAccounts = FourtwentyAccounts.find()
    .fetch()
  ...
  _.each(walletsAndContracts, function(account) {
    if (account.address) {
      web3.fourtwenty.getBalance(account.address, function(err, res) {
        if (!err) {
          // is of type wallet
  ...
  // issue found when using the light client mode on Haze 0.9.1 and 0.9.2
...
observeLatestBlocks = function() {
  ...
  web3.fourtwenty.subscribe('newBlockHeaders', function(e, res) {
    if (!e) {
      updateBalances();
    }
  });
